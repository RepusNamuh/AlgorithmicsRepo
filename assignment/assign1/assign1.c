...
///////////////////////////////////////////////////////////////////////
void getTotalCandidates(pollStatus_t *stat);

void getCandidatesName(candidate_t votingPool[], pollStatus_t *stat);
...
// Read the total number of candidates from the input buffer
void getTotalCandidates(pollStatus_t *stat) {
    //Reading the input
...
// Read the first line containing the names of candidates
void getCandidatesName(candidate_t votingPool[], pollStatus_t *stat) {
    int i = 0;
    candidate_t dummy;
...
// Shift every element to the right of eliminated candidate left by one
void shiftLeft(candidate_t votingPool[], int start, int *end) {
...
int elimination(candidate_t votingPool[], pollStatus_t *stat, int *winner) {
    ...
    for (int candidate = 0; candidate < stat->remainingCandidates; 
                                                            candidate++) {
        ...
        // Searching for potential outright winner
...
void statusPrintOut(candidate_t votingPool[], int elimIndex, 
    ...
    // After the previous round of redistrubtution.
    if (*winner != NOTFOUND) {
...
void redistributeVotes(candidate_t votingPool[], pollStatus_t *stat) {
    ...
    // 1 if highest then 2, 3, 4, ...
    // To include preferences of eliminated candidates
    // e.g, after second round of elimination,
    // we search through the votes of the candidate and redistribute
    // ballot that have this candidate as 1st and 2nd preference.
    // So formula to consider this is: after n rounds of elimination,
    // we need only consider redistribution of ballots with highest
    // preference of n. Which is simply the declaration below:
    ...
    for (int i = 0; i < stat->totalVotes; i++) {
        ...
        //flag to indicate a higher preference in the ballot than
        // highestPreft
        int higherFound = FALSE; 

        for (int j = 0; j < stat->remainingCandidates;j++) {
            // Within this particular ballot,
            // if there is a candidate with a higher preference
            // than the current eliminated candidate,
            // then no redistribution is needed for this ballot
...
// Print out the stage header for each stage.
void stageHeader(int stage) {
    printf("Stage %d\n", stage);
    printf("=======\n");
}
